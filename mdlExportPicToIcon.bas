Attribute VB_Name = "mdlExportPicToIcon"
Option Explicit
...
Public Function ExportPicToIcon(PicturehDC As Long, TransperantColor As Long, ExportFileName As String, SaveBits As IconSaveTypes) As Boolean
Dim RET, bmpPicInfo As BITMAPINFO
    TransCol = TransperantColor
    ...
    RET = BitBlt(IconInfo.iDC, 0, 0, 32, 32, PicturehDC, 0, 0, vbSrcCopy)
    If RET = 0 Then
    ExportPicToIcon = False
    Exit Function
    End If
    DoEvents
    SaveIcon ExportFileName, IconInfo.iDC, IconInfo.iBitmap, SaveBits
    IconInfo.iFileName = ExportFileName
    DeleteDC IconInfo.iDC
    DeleteObject IconInfo.iBitmap
    Screen.MousePointer = vbDefault
    DoEvents
    ExportPicToIcon = True
End Function
